...
async function loadDynamicPortfolioData() {
    ...
    try {
        ...
        const profileImg = document.getElementById("profileImageEl");
        if (profileImg && profile.image) {
            profileImg.src = fullUrl(profile.image);
        }

        const profileName = document.querySelector(".profile-box h2");
        if (profileName && profile.name) profileName.textContent = profile.name;

        const profileRole = document.querySelector(".profile-box p");
        if (profileRole && profile.title) profileRole.textContent = profile.title;
        ...
        if (heroRole && profile.title) heroRole.textContent = profile.title;

        const aboutText = document.querySelector(".about-text");
        if (aboutText && data.about) {
            aboutText.innerHTML = `<p>${data.about}</p>`;
        }

        if (data.resumeUrl) {
            const resumeUrl = fullUrl(data.resumeUrl);

            document.querySelectorAll('a[href*="Amrit_Raj_Resume.pdf"]').forEach(link => {
                link.href = resumeUrl;
            });

            const resumeFrame = document.querySelector(".resume-preview-box iframe");
        ...
        if (data.projects && data.projects.length > 0) {
            ...
            const projectImgUrl = project.image || project.image_url;

            if (projectImage && projectImgUrl) {
                projectImage.src = fullUrl(projectImgUrl);
            }

            const projectTitle = document.querySelector(".project-content h3");
            if (projectTitle && project.title) projectTitle.textContent = project.title;

            const projectDesc = document.querySelector(".project-content p");
            if (projectDesc && project.description) projectDesc.textContent = project.description;

            const stackText = document.querySelector(".stack p");
            if (stackText && project.stack) stackText.textContent = project.stack;
        }
        ...
        if (data.certificates && data.certificates.length > 0) {
            ...
            if (certificateGrid) {
                certificateGrid.innerHTML = data.certificates.map(cert => `
                    <div class="certificate-card reveal active">
                        ${cert.image ? `<img src="${fullUrl(cert.image)}" alt="${cert.title || "Certificate"}">` : ""}
                        <h3>${cert.title || ""}</h3>
                        <p>${cert.issuer || ""}</p>
                        ${cert.url ? `<a href="${cert.url}" target="_blank" class="btn secondary">View Certificate</a>` : ""}
                    </div>
                `).join("");
            }
        }
        ...
        console.error("Portfolio dynamic data error:", error);
    }
}

document.addEventListener("DOMContentLoaded", loadDynamicPortfolioData);